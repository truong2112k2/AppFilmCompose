...
class SearchMovieViewModel @Inject constructor(
    ...
    private val _searchMoviesState = MutableStateFlow(UIState())
    ...
    private fun getMoviesSearch(keyword: String) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            _searchMoviesState.value = UIState(isLoading = true)

            if (keyword.isEmpty()) {
                _searchMoviesState.value = UIState(error = "Input name movie, please!")
            ...
            _searchMoviesState.value = when (searchMovies) {
                is Resource.Error -> {
                    ...
                    UIState(error = searchMovies.message)
                ...
                is Resource.Success -> {
                    ...
                    if (searchMovies.data != null) {
                        ...
                        if (searchMovies.data.isEmpty()) {
                            _searchMoviesState.value = UIState(error = "No result!")
                        ...
                        Log.d(
                            ...
                            "searchMovies movies by SearchMovieViewModel success ${searchMovies.data.size}"
                        )

                        Log.d(
                            Constants.STATUS_TAG,
                            "_listMovieSearch movies by SearchMovieViewModel success ${_listMovieSearch.value.size}"
                        )
    ...
    private fun resetSearchState() {
        _searchMoviesState.value = UIState()
    }
